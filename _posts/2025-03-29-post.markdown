...
In his essay [*On Writing*](https://en.wikipedia.org/wiki/On_Writing:_A_Memoir_of_the_Craft), Stephen King advises writers to begin with a "situation" rather than a fully fleshed-out plot. The idea is to take a simple scenario, like a woman alone in her house with her ex-husband circling outside in the dark, and then "unpack" it to see where the story leads. At first glance, programming might seem worlds away from such tension-filled moments. Yet, if you look beneath the surface, you'll find a different kind of narrative at play. As [Pieter Hintjens](https://en.wikipedia.org/wiki/Pieter_Hintjens) points out in his excellent [*The Guide*](https://zguide.zeromq.org/), the world of software development is shaped by very human forces: [stupidity, selfishness, jealousy, and fear](https://zguide.zeromq.org/docs/chapter6/#Psychology-of-Software-Architecture). In my case, jealousy is at work and and this blog post is about trying to outdo my coworkers on a small coding challenge we use in interviews. A couple of years ago, each team members gave its best on the coding challenge and my implementation was not the fastest. It was the one with the lower memory usage, if I remember well, but it was slow. Now my goal: take the fastest C++ code the team produced, re-implement the same algorithm using my (very modest) beginner Rust skills, and see if I can beat C++. I am the husband and I have a rusty knife in the hand.

## Minesweeper
An image may be better than a thousand words: 
...
```
That may be slow if $$B$$ is large (close to $$N^2$$.. think of the extreme case where the grid is full of bombs). There is a trick: if $$B>\frac{N^2}{2}$$ we can invert the logic and generate the bomb-free positions instead of bomb positions. 

I wanted a fast, deterministic algorithm that prioritizes [time](https://en.wikipedia.org/wiki/Time_complexity) over [space](https://en.wikipedia.org/wiki/Space_complexity) complexity, and chose the Fisher–Yates shuffle. This algorithm makes $$B$$ *distincts* random numbers out of any $$B$$ random numbers, distincts or not. The tradeoff is that it has $$O(N^2)$$ space complexity (memory requirement).
...
```
There are many `if` branches. 

### Printing the grid

Printing using `print!` or `println!` is [slow](https://users.rust-lang.org/t/efficiency-of-println-and-format/52772). It is much faster to write bytes directly to stdout. I convert the grid elements to printable characters with a lookup table, and print line-by-line:
```
...
The measurments are done on a debian (bookworm) box with a i7-4470 CPU and 32Gb RAM. The output of the processes under test is redirect to `/dev/null`.
...
The performance delta gets larger, for example with $$B=N^2/8$$:

| ![tile](/assets/minesweeper/time-array-baseline-8.png) |

I don't know why. This is a bit scary. I fear there is an error in the code, but I don't see. If you see an error, please drop me an email. Maybe the rust compiler just tries harder to optimize (which takes more time). I don't know.

At this point I could call it a day and move along, but I was caught and wanted to make my minesweeper code faster with multithreading. [Fearless Concurrency](https://doc.rust-lang.org/book/ch16-00-concurrency.html) is a big selling point of rust. 

## Grid With Margin

As a first step to a concurrent algorithm, I pad the grid with margins. 

| ![tile](/assets/minesweeper/margins.png) |
|:----------------------------------------:|
| Grid padded with margins                 |

This will allow us in the second step to split the grid into many subgrids that can be process independantly, without loosing neighboors counts. This way, we avoid using mutexes, and as a bonus we can remove a couple of `ifs` (in both rust and C++) and indeed this alone makes the algorithms faster (less branches). Before leaving C++ behind, I look it its perforance with the margin algorithm.
...
The idea is very simple: divide the large minesweeper grids into many small rectangular tiles (subgrids), as shown here:

| ![tile](/assets/minesweeper/tiles.png) |
|:--------------------------------------:|
| Tiles for N=6,K=3 => X=6, Y=2          |

If $$K$$ is the number of tiles, each tile has size $$(X+2)x(Y+2)$$ where $$X=N$$ and $$Y=N/K$$.
...
Probably because the bomb generation is spread over time: Assuming chunk 2 is processed first, and chunk 3 is generated at a later time, the chunk 2 memory is freed when chunk 3 starts allocating memory. The maximum amount of memory allocated at any given time decreases.

## Notes

### Where is the code ?

The C++ code is [here](https://github.com/Julien5/sandbox/blob/master/test/rust/minesweeper/C/), the rust code is [here](https://github.com/Julien5/sandbox/blob/master/test/rust/minesweeper/src/). The script that produces the picture is [here](https://github.com/Julien5/sandbox/blob/master/test/rust/minesweeper/measurement/main.sh).

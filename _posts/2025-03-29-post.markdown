...
### Baseline against Parallel

First we check that the parallel version with one chunk (K=1, i.e. one tile, one thread, no parallelism) is *not* slower than the baseline.
...
| L  | D | baseline (time in s) | parallel (time in s) |
|----|---|----------------------|----------------------|
| 4  | 2 | 1.35                 | 1.16                 |
| 8  | 2 | 5.65                 | 5.00                 |
| 10 | 2 | 9.09                 | 7.81                 |

It is not only *not* slower, it is a little faster. This is because with parallel version has margins and does not need to check boundaries: faster code with less branches. I checked that the baseline get similar timing to the parallel algorithm by manipulating it. If no bombs is on the grid edges and the bound checking is removed, the timings are similar.
...
The C++ implementation has margins and only one tile (K=1). So we set K=1 in the parallel version.
Again D=2.
Here are the timings (logscale with `log2`).

| ![tile](/assets/minesweeper/time-array-2.png) |
|:---------------------------------------------:|
| Time as a function of L                       |

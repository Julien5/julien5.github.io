...
This will allow us, in the second step, to divide the grid into multiple subgrids that can be process independantly without loosing neighboors counts. By doing so, we avoid the need for mutexes, and as a bonus we can eliminate a couple of `ifs`. This alone  makes the code faster (in both rust and C++). Before leaving C++ behind, lets look at the performance with the margin algorithm.
...
If $$K$$ is the number of tiles, each tile has dimension $$(X+2)x(Y+2)$$ where $$X=N$$ and $$Y=N/K$$.
